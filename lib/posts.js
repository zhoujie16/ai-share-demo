...
function publicPost(post) {
  ...
  return {
    ...
    authorNickname: rest.isAnonymous ? ANONYMOUS_AUTHOR : cleanText(rest.authorNickname) || ANONYMOUS_AUTHOR,
    isAnonymous: Boolean(rest.isAnonymous) || !cleanText(rest.authorNickname),
...
export async function createPost(input) {
  ...
  const authorNickname = isAnonymous ? ANONYMOUS_AUTHOR : cleanText(input.authorNickname);
  ...
  if (!isAnonymous && !authorNickname) {
    const error = new Error("请选择匿名或填写作者昵称");
    error.status = 400;
    throw error;
  }
  ...
  const post = {
    ...
    body,
    authorNickname: authorNickname.slice(0, 20),
    isAnonymous,
